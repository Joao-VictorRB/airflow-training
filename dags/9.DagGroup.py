import pendulum
from airflow import DAG
from airflow.operators.bash import BashOperator
from airflow.utils.task_group import TaskGroup


with DAG(

    dag_id = "dag_group",
    description = "dag group",
    schedule = None,
    start_date = pendulum.datetime(2025,1,1,tz="America/Sao_Paulo"),
    catchup = False,
    tags = ["curso","exemplo","complexa"]
) as dag:

    task1 = BashOperator(task_id='tsk1',bash_command="sleep 1")
    task2 = BashOperator(task_id='tsk2',bash_command="sleep 2")
    task3 = BashOperator(task_id='tsk3',bash_command="sleep 1")
    task4 = BashOperator(task_id='tsk4',bash_command="sleep 2")
    task5 = BashOperator(task_id='tsk5',bash_command="sleep 6")
    task6 = BashOperator(task_id='tsk6',bash_command="sleep 6")

    with TaskGroup(group_id ="tks_group") as tks_group:
        task7 = BashOperator(task_id='tsk7',bash_command="sleep 4")
        task8 = BashOperator(task_id='tsk8',bash_command="sleep 4")
        task9 = BashOperator(task_id='tsk9',bash_command="sleep 4")
    
    task1 >> task2
    task3 >> task4

    [task2, task4] >> task5 >> task6 >> tks_group
    